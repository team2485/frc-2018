...
public class Arm extends Subsystem {
	...
	public static final double CRITICAL_DISTANCE = toMeters(42); // temp //distance from mast to 16 inches past frame
																	// perimeter
	public static final double CONSERVATIVE_CRITICAL_DISTANCE = toMeters(38);
	public static final double QUASI_CONSERVATIVE_CRITICAL_DISTANCE = toMeters(38);
	...
	public static final double MIN_WRIST_LIFTING_POSITION = 15;
	...
	public static enum ArmSetpoint {
		INTAKE(-.192, -0.030), SWITCH(-.192, 0.15), SECOND_STORY(-.192, 0.025), SCALE_HIGH_BACK(0.22,
				0.4), SCALE_MIDDLE_BACK(0.2,
						0.46), SCALE_LOW_BACK(0.16, 0.52), SEVEN_FOOT_SCALE(.22, .32), CLIMB(.26, .225);
	...
	private static final double MAX_WRIST_ANGLE = .375;
	...
	public double getIMinElbow() {
		...
		if (RobotMap.wristEncoderWrapperDistance.pidGet() > MAX_WRIST_ANGLE) { // if wrist at hard stop don't move arm
																				// down
			return -2;
		}
		double i = RobotMap.elbowTalon.getOutputCurrent();
	...
	public double getIMaxWrist() {
		if (Math.abs(RobotMap.wristEncoderWrapperRate.pidGet()) < LOW_ENC_RATE
				&& Math.abs(RobotMap.elbowEncoderWrapperRate.pidGet()) < QUASI_LOW_ELBOW_RATE) {
			return 4;
		}
	...
	public Arm() {
		...
		MIN_WRIST_ANGLE_CROSS = FastMath.acos((CRITICAL_DISTANCE * CRITICAL_DISTANCE - L2 * L2 - L1 * L1) / 2 / L1 / L2)
				/ 2 / Math.PI;
		MID_ELBOW_ANGLE = -FastMath
				.acos((L1 * L1 + CRITICAL_DISTANCE * CRITICAL_DISTANCE - L2 * L2) / 2 / CRITICAL_DISTANCE / L1) / 2
				/ Math.PI;
		...
		elbowMinAngVelSource.setPidSource(() -> {
			// return isClimb ? -.15 : -.3;
			double minAngVel = RobotMap.wristEncoderWrapperDistance.pidGet() > MAX_WRIST_ANGLE ? -.25 : -.5;
			return isClimb ? -.15 : minAngVel;
		});
		...
		wristOutputSource.setPidOutput((double out) -> {
			double pwm = out; // + ConstantsIO.levitateWristCurrent * FastMath.cos(2 * Math.PI *
								// getWristAngle());
	...
	public boolean isElbowCurrentSensorWorking() {
		boolean isElbowTalonWorking = RobotMap.elbowTalon.getOutputCurrent() == 0
				&& RobotMap.elbowTalon.getMotorOutputPercent() > MAX_PWM_THRESHOLD_NO_CURRENT;
		return !isElbowTalonWorking;
	}

	public boolean isWristCurrentSensorWorking() {
		boolean isWristTalonWorking = RobotMap.wristTalon.getOutputCurrent() == 0
				&& RobotMap.wristTalon.getMotorOutputPercent() > MAX_PWM_THRESHOLD_NO_CURRENT;
	...
	public double getMinWristPos() {
		double margin = (QUASI_CONSERVATIVE_CRITICAL_DISTANCE - (L1 * FastMath.cos(getElbowAngle() * 2 * Math.PI)))
				/ L2;
		return margin > 1 || Math.abs(getElbowAngle()) > CRITICAL_ANGLE ? -100
				: FastMath.acos(margin) / 2 / Math.PI - RobotMap.elbowEncoderWrapperDistance.pidGet();
	}
	...
	public void updateConstants() {
		...
		// wristAngVelMaxPID.setPID(ConstantsIO.kP_WristAngVel,
		// ConstantsIO.kI_WristAngVel, 0, ConstantsIO.kF_WristAngVel);
		// wristAngVelMinPID.setPID(ConstantsIO.kP_WristAngVel,
		// ConstantsIO.kI_WristAngVel, 0, ConstantsIO.kF_WristAngVel);
